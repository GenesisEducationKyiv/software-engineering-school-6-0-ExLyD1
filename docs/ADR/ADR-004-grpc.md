...
### Результати (50 conn, 20s, localhost, 1 теплий рядок)

| Метрика | REST/JSON | gRPC/protobuf |
| --- | --- | --- |
| Throughput (req/s) | 8 177 | **9 042** (+10.6%) |
| Latency p50 (ms) | 4 | 4.0 |
| Latency p95 (ms) | ~18 (p97.5) | **12.2** |
| Latency p99 (ms) | 26 | 26.7 |
| Тіло відповіді (bytes) | ~49 (JSON) | **~21** (protobuf) |

**Висновок:** на loopback із крихітним payload gRPC помірно випереджає REST по throughput і кращий на хвостах (p95), а тіло відповіді — у ~2.3× менше. Різниця скромна саме тому, що це localhost + мікроскопічне повідомлення (домінує CPU-серіалізація, а не мережа); на реальній мережі та більших payload перевага protobuf+HTTP/2 росла б.

**Урок про чесність бенчмарку (реально стався):** перший прогін `ghz` я зробив **у Docker-контейнері** через `host.docker.internal` (Docker NAT на macOS) + amd64-емуляція на arm64 — і gRPC «програв» (3.9k проти 8.2k req/s). Це був покалічений **клієнт**, а не повільний протокол. Коректний замір — нативний `ghz` на хості, в однакових із `autocannon` умовах. Мораль: однакове середовище для обох клієнтів важливіше за самі цифри.

`autocannon` = HTTP/1.1 з N зʼєднань; gRPC = HTTP/2 з мультиплексуванням над меншою к-стю — це частина суті протоколів, не баг тесту.
